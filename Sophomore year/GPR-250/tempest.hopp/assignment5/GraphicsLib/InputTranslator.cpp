...
void InputTranslator::handleEvent(const Event& theEvent)
{
	if (theEvent.getType() == MOUSE_PRESSED)
	{
		...
		{
			AddUnitEvent addUnitEvent = AddUnitEvent(mousePressedEvent.getLocation());
			EventSystem::getInstance()->fireEvent(addUnitEvent);
		}
		else if (mousePressedEvent.getButton() == Input::MouseButton::RIGHT)
		{
			DelUnitEvent deleteUnitEvent = DelUnitEvent(mousePressedEvent.getLocation());
			EventSystem::getInstance()->fireEvent(deleteUnitEvent);
		}
	}
